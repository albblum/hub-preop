...
import { formalApprovalBodySchema } from "@/lib/validation/committee";
import { handleDomainError, handleIntegrityError } from "@/lib/api-instrument";
import { prisma } from "@/lib/prisma";

type RouteContext = { params: Promise<{ id: string }> };

/** Aprovação formal com quórum MVP e fundamento normativo registado. */
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = formalApprovalBodySchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const updated = await committeeFormalApproval({
      instrumentId: id,
      foundationNote: parsed.data.foundationNote,
